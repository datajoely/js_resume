...
async function generatePDF() {
  try {
    ...
    // Wait for fonts to load and CSS to be fully applied
    await page.evaluate(() => {
      return new Promise((resolve) => {
        // Wait for all fonts to load
        if (document.fonts && document.fonts.ready) {
          document.fonts.ready.then(() => {
            // Additional wait to ensure CSS is fully applied
    ...
    // Remove existing PDF if it exists to ensure fresh generation
    ...
    // Verify the PDF was created and get its size
    const stats = fs.statSync(pdfPath);
    console.log(`PDF generated successfully: docs/resume.pdf (${stats.size} bytes)`);
